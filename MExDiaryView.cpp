...
BEGIN_MESSAGE_MAP(CMExDiaryView, CFormView)
	...
	ON_WM_CONTEXTMENU()
	ON_WM_RBUTTONUP()
	ON_EN_CHANGE(IDC_DATE, &CMExDiaryView::OnEnChangeDate)
	ON_EN_CHANGE(IDC_SUBJECT, &CMExDiaryView::OnEnChangeSubject)
//	ON_EN_CHANGE(IDC_CONTENT, &CMExDiaryView::OnEnChangeContent)
...
void CMExDiaryView::OnEnChangeDate()
{
	UpdateData(TRUE);
	CMExDiaryDoc* pDoc = (CMExDiaryDoc*)GetDocument();
...
void CMExDiaryView::OnEnChangeSubject()
{
	UpdateData(TRUE);
	CMExDiaryDoc* pDoc = (CMExDiaryDoc*)GetDocument();
